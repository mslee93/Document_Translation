import os
import time
import translate_ppt, translate_excel
import translate_pb2

translate_dic = {'pptx': translate_ppt, 'xlsx': translate_excel}
...
class FolderControl(object):
    ...
    input_folder_path = str(os.path.join(os.getcwd(), 'Input_Temp'))
    output_folder_path = str(os.path.join(os.getcwd(), 'Output_Temp'))
...
class TRANSLATE(File):
    def __init__(self, job_id, file_name, file_data, src_lang, tgt_lang, eng):
        super(TRANSLATE, self).__init__(job_id, file_name, file_data)
        self.src_lang = src_lang
        self.tgt_lang = tgt_lang
        self.eng = eng
        self.ext = os.path.splitext(os.path.basename(self.temp_file_name))[-1].replace('.', '')
        self.output_file_path = None

    def run(self):
        temp_output_file_path = os.path.join(FolderControl.output_folder_path, os.path.splitext(os.path.basename(self.temp_file_name))[0]+'.'+self.ext)
        translate_dic[self.ext].run(self.input_file_path, temp_output_file_path, self.src_lang, self.tgt_lang, self.eng)
        self.output_file_path = temp_output_file_path
        super(TRANSLATE, self).deleteInputFile()
...
def main(job_id, file_name, file_data, src_lang, tgt_lang, eng):

    task = TRANSLATE(job_id, file_name, file_data, src_lang, tgt_lang, eng)
    task.run()
    return_value = translate_pb2.translateResponse(file_data=task.result())
