...
def translate(shape, src_lang, tgt_lang, engine):
...
# 테이블 안에 있는 문자열 번역
def table_func(shape, src_lang, tgt_lang, engine):
    table = shape.table
    # 셀단위로 체크
    for cell in table.iter_cells():
        translate(cell, src_lang, tgt_lang, engine)


# 텍스트 상자 문자열 번역
def text_func(shape, src_lang, tgt_lang, engine):
    translate(shape, src_lang, tgt_lang, engine)


def chart_func(shape, src_lang, tgt_lang, engine):
    chart = shape.chart
    if chart.has_title:
        if chart.chart_title.has_text_frame:
            translate(chart.chart_title, src_lang, tgt_lang, engine)
    try:
        if chart.value_axis.has_title:
            if chart.value_axis.axis_title.has_text_frame:
                translate(chart.value_axis.axis_title, src_lang, tgt_lang, engine)
    except: 
        pass
    try:
        if chart.category_axis.has_title:
            if chart.category_axis.axis_title.has_text_frame:
                translate(chart.category_axis.axis_title, src_lang, tgt_lang, engine)
    except:
        pass
...
def handle_shape(prs, tgt_file, shape, src_lang, tgt_lang, engine):
    if shape.shape_type == MSO_SHAPE_TYPE.GROUP:
        for _shape in shape.shapes:
            handle_shape(prs, tgt_file, _shape, src_lang, tgt_lang, engine)
    # 객체가 테이블이면
    elif shape.has_table:
        # 테이블 번역 함수 호출
        table_func(shape, src_lang, tgt_lang, engine)
    ...
    # 텍스트 상자면
        ...
        # 텍스트 번역 함수 호출
        text_func(shape, src_lang, tgt_lang, engine)
            # 번역함수에서 리턴이 False라면 웹 API에 이상으로 지금까지 작업한 내용 저장하고 리턴
            # prs.save(tgt_file)
            # return False
    elif shape.has_chart:
        chart_func(shape, src_lang, tgt_lang, engine)
    ...
    if engine is not None:
        ...
        for slide in prs.slides:
            ...
            for shape in slide.shapes:
                handle_shape(prs, tgt_file, shape, src_lang, tgt_lang, engine)
